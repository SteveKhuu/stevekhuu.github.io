...
curl --header "Circle-Token: $CIRCLE_API_TOKEN" --request GET "https://circleci.com/api/v2/workflow/${CIRCLE_WORKFLOW_ID}" -o current_workflow.json
...
PIPE_IDS=$(curl --header "Circle-Token: $CIRCLE_API_TOKEN" --request GET "https://circleci.com/api/v2/project/gh/$CIRCLE_PROJECT_USERNAME/$CIRCLE_PROJECT_REPONAME/pipeline?branch=$CIRCLE_BRANCH"|jq -r --argjson CURRENT_PIPELINE_NUM "$CURRENT_PIPELINE_NUM" --arg TIME_THRESHOLD "${TIME_THRESHOLD}" '.items[] | select(.state == "created") | select(.number < $CURRENT_PIPELINE_NUM) | select(.created_at > $TIME_THRESHOLD) | .id')
...
if [ ! -z "$PIPE_IDS" ]; then
  ...
  do
    curl --header "Circle-Token: $CIRCLE_API_TOKEN" --request GET "https://circleci.com/api/v2/pipeline/${PIPE_ID}/workflow"|jq -r --arg WF_NAME "${WF_NAME}" '.items[]|select(.status == "on_hold" or .status == "running") | select(.name == $WF_NAME) | .id' >> WF_to_cancel.txt
  done
fi
...
if [ -s WF_to_cancel.txt ]; then
  ...
  while read WF_ID;
    do
      curl --header "Circle-Token: $CIRCLE_API_TOKEN" --request POST https://circleci.com/api/v2/workflow/$WF_ID/cancel
